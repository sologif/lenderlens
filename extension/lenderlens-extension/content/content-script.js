...
(function () {
  ...
  // Point at the deployed backend, not a local dev server.
  const BACKEND_ORIGIN = 'https://lenderlens-9rky.onrender.com';
  const API_ENDPOINT = `${BACKEND_ORIGIN}/api/analyze`;
  const PAYMENT_ENDPOINT = `${BACKEND_ORIGIN}/api/payment-analyze`;
  ...
  const TRUST_CARD_STYLE = `
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .ll-logo { width: 16px; height: 16px; display: block; }
    .ll-close {
      background: none;
      border: none;
      font-size: 16px;
  ...
  function renderInPageTrustCard(data, isPayment = false) {
    ...
    card.innerHTML = `
      <div class="ll-header">
        <div class="ll-brand">
          <img src="${logoUrl}" alt="LenderLens" class="ll-logo">
    ...
    shadowRoot.getElementById('ll-btn-evidence').addEventListener('click', () => {
      window.open(`${BACKEND_ORIGIN}/dashboard/index.html`, '_blank');
    });
    shadowRoot.getElementById('ll-btn-report').addEventListener('click', () => {
      fetch(`${BACKEND_ORIGIN}/api/cases/report`, {
        method: 'POST',
